...
#################################################################
 ...
 Res2b[q,] <- ResS(sim_rs(N=Pop[q],Rate=1.0, L=0.2, U=1,et=1,es=0.39))
 Res3b[q,] <- ResS(sim_rs(N=Pop[q],Rate=1.0, L=0.2, U=1,et=1,es=0.79))

 Res4b[q,] <- ResS(sim_rs(N=Pop[q],Rate=7.0, L=0.2, U=1,et=1,es=0.01))
 Res5b[q,] <- ResS(sim_rs(N=Pop[q],Rate=7.0, L=0.2, U=1,et=1,es=0.39))
 Res6b[q,] <- ResS(sim_rs(N=Pop[q],Rate=7.0, L=0.2, U=1,et=1,es=0.79))
 
 Res7b[q,] <- ResS(sim_rs(N=Pop[q],Rate=20.0, L=0.2, U=1,et=1,es=0.01))
 Res8b[q,] <- ResS(sim_rs(N=Pop[q],Rate=20.0, L=0.2, U=1,et=1,es=0.39))
 Res9b[q,] <- ResS(sim_rs(N=Pop[q],Rate=20.0, L=0.2, U=1,et=1,es=0.79))
              }
 ...
 dat2x <- dat1x#[which(dat1x$Lab2=="Low Mean"),]
 ...
 stat_density_ridges(data= dat1x.p, aes(x=M, y=factor(Lab4), fill=0.5 - abs(0.5-..ecdf..)),
  ...
  facet_wrap(Lab2~Lab1,scales="fixed") +  ylab("Sample size")  + coord_cartesian(xlim = c(-1.5, 2.7), expand = FALSE)
    ...
    ggsave("BayesCheck.pdf",p1,height=10,width=12)



x<-sim_rs(N=Pop[1],Rate=7.0, L=0.2, U=1,et=1,es=0.39)
           r <- x$r
           t <- x$t
M_index_stan(r,t, chains=4,samples=6000,warmup=4000,adapt_delta=0.99) 
StanResults_Check1 <- StanResults
ggsave("Trace_C_1.pdf",traceplot(StanResults_Check1,pars=c("gamma", "Concentration", "M_raw", "M", "M_raw_age", "M_age","alpha[1]","alpha[5]","alpha[10]")),height=4,width=12)   

x<-sim_rs(N=Pop[3],Rate=7.0, L=0.2, U=1,et=1,es=0.39)
           r <- x$r
           t <- x$t
M_index_stan(r,t, chains=4,samples=6000,warmup=4000,adapt_delta=0.99) 
StanResults_Check2 <- StanResults
ggsave("Trace_C_2.pdf",traceplot(StanResults_Check2,pars=c("gamma", "Concentration", "M_raw", "M", "M_raw_age", "M_age","alpha[1]","alpha[5]","alpha[10]")),height=4,width=12)   

x<-sim_rs(N=Pop[5],Rate=7.0, L=0.2, U=1,et=1,es=0.39)
           r <- x$r
           t <- x$t
M_index_stan(r,t, chains=4,samples=6000,warmup=4000,adapt_delta=0.99) 
StanResults_Check3 <- StanResults
ggsave("Trace_C_3.pdf",traceplot(StanResults_Check3,pars=c("gamma", "Concentration", "M_raw", "M", "M_raw_age", "M_age","alpha[1]","alpha[5]","alpha[10]")),height=4,width=12)   